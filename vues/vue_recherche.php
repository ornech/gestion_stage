...
function generateLink() {
    ...
    const fullUrl = `${baseUrl}?terme=&cp_dep_label=${encodeURIComponent(departementLabel)}%29&cp_dep_type=dep&cp_dep=${encodeURIComponent(departementCode)}&${selectedNafs.map(naf => `naf=${naf}`).join('&')}&fn=&n=&dmin=&dmax=&type=&label=&etat=&sap=`;
